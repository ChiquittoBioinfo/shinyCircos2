...
data.C.name <- "size-sg_C.csv"
data.C <- data.frame(fread(data.C.name),stringsAsFactors=F)
data.C[,2] <- as.numeric(data.C[,2])
data.C[,3] <- as.numeric(data.C[,3])
data.T.file <- c("sg_C_genes.csv","sg_C_TE.csv","sg_C_car_final_ncRNAs_lista.cl.gff3_ALL-stack.csv","sg_C_car_final_ncRNAs_lista.cl.gff3_MIRMA.csv","sg_C_car_final_ncRNAs_lista.cl.gff3_SRNA.csv")
...
colorChr <- c("white","white","white","white","white","white","white","white","white","white","white")
heightChr <- 0.05
...
takindx <- 1
typeTrack <- c("line","line","point","point","point")
i <- 1
data.TT <- data.T[[i]]
	tktype <- typeTrack[i]
...
transparencyHlt <- c(1,1,1,1,1)
lkmargin <- 0
tkborder <- NA
columns <- c(4,5,6,7)
selreaTrack <- c(1,1,1,1,1)
fillareaTrack="add"
fillareaTrack="add"
fillareaTrack=""
fillareaTrack=""
...
poslabels <- c("outer","outer","outer","outer","outer")
...
ytop <- 0.14714
len <- 0.3
gap <- 0.0714285714285714
legendtext <- c("Genes","TEs","All NCRNA","miRNA","sRNA")
for(i in 1:n){
